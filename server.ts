...
// In-memory mock DB for HR API Sync & Realtime Node persistence
let orgNodesStore = [
  {
    id: "node-1",
...
app.get("/api/health", (req, res) => {
  res.json({ status: "ok", timestamp: new Date().toISOString(), totalNodes: orgNodesStore.length });
});

// Get organigram nodes
app.get("/api/v1/nodes", (req, res) => {
  const { sede, department } = req.query;
  let filtered = [...orgNodesStore];
  if (sede && sede !== "all") {
...
app.post("/api/v1/nodes/bulk-sync", (req, res) => {
  ...
  if (Array.isArray(nodes)) {
    ...
    return res.json({ success: true, count: orgNodesStore.length });
  }
...
app.post("/api/v1/hr/sync", (req, res) => {
  ...
  if (incomingEmployees.length > 0) {
    ...
    const mapped = incomingEmployees.map((emp: any, index: number) => ({
      ...
      parentId: emp.reportsToId || emp.parentId || orgNodesStore[0]?.id || null,
    ...
    if (req.body.mode === "replace") {
      orgNodesStore = mapped;
    } else {
      // merge without duplicating IDs
      const existingIds = new Set(orgNodesStore.map((n) => n.id));
      mapped.forEach((m: any) => {
        if (!existingIds.has(m.id)) {
          orgNodesStore.push(m);
        }
      });
    }
    ...
    return res.json({
      ...
      currentTotalNodes: orgNodesStore.length
    });
  }
