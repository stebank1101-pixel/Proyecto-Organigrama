...
export const FreeView = forwardRef<HTMLDivElement, FreeViewProps>(function FreeView(
  ...
  const dragState = useRef<{ id: string; offsetX: number; offsetY: number; scale: number } | null>(null);
  ...
  function handlePointerDown(e: React.PointerEvent, node: OrgNode) {
    ...
    const scale = containerEl.getBoundingClientRect().width / containerEl.scrollWidth || 1;
    dragState.current = {
      id: node.id,
      offsetX: e.clientX / scale - node.freeX,
      offsetY: e.clientY / scale - node.freeY,
      scale,
    };
  ...
  function handlePointerMove(e: React.PointerEvent) {
    ...
    if (!node) return;
    const nextX = Math.max(0, e.clientX / drag.scale - drag.offsetX);
    const nextY = Math.max(0, e.clientY / drag.scale - drag.offsetY);
    node.freeX = nextX;
    node.freeY = nextY;
    forceRerender((v) => v + 1);
  }
